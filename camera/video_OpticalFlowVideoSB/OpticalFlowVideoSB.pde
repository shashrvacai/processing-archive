...
public void setup() {
  ...
  // webcam capture
  movie = new Movie(this, "insaan.mov");
  movie.play();
  movie.loop();
  // render buffers
  pg_cam_a = (PGraphics2D) createGraphics(cam_w, cam_h+100, P2D);
  pg_cam_a.noSmooth();
  pg_cam_a.beginDraw();
...
public void draw() {
  ...
  if ( movie.available() ) {
    ...
    pg_cam_b.image(movie,0, 200,view_w/2,view_h/2);    /// position and size of the fluid object 
    pg_cam_b.endDraw();
