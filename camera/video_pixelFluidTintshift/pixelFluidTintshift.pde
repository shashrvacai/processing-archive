...
import processing.video.Capture;
  ...
  public void setup() {    
    ...
    cam = new Capture(this, cam_w, cam_h);           // webcam capture
    cam.start();
  ...
  public void draw() {
    ...
    if( cam.available() ){
      ...
      pg_cam_b.tint(int(r),int(g),int(b));    //  tint to select colors 
      pg_cam_b.endDraw();
   ...
   if (r > 0 ){     r -= 0.01 ;   } else {     r = 255 ;    }
   if (g > 0 ){     g -= 0.2 ;   } else {     g = 255 ;    }
   if (b > 0 ){     b -= 1 ;   } else {     b = 255 ;    }
