  ...
  for (var i = 0; i < constellation.length; i++) {
    ...
    for (var j = 0; j < constellation.length; j++) {
      if (i > j) { // "if (i > j)" => to check one time distance between two stars
        ...
        if (d >= 350 && d<= 550) { // if d is less than width/10 px, we draw a line between the two stars
...
function star() {
  ...
  this.a =map(mouseX,0,width,2*PI,-2*PI); //random(5 * TAU); // "5*TAU" => render will be more homogeneous
  ...
  this.update = function() {
      ...
      //this.bam.random2D();
      this.bam.mult(0.00001);
      this.speed.add(this.bam);
      // speed is done according distance between loc and the mouse :
      this.m = constrain(map(dist(this.loc.x, this.loc.y, width/2, height/2),width, width, 8, .05), .05, 8); // constrain => avoid returning "not a number"
      this.speed.normalize().mult(5);
