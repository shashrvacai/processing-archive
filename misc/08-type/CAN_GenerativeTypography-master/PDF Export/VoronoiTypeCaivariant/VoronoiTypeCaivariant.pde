...
void setup() {
  ...
  RFont font = new RFont("../../Fonts/FreeSans.ttf", 350);       // create font used by Geomerative
  shape = font.toShape("Cai");                    // create base shape from text using the loaded font
  shape.translate(width/2 - shape.getWidth()/2, height/2 + shape.getHeight()/2);     // center the shape in the middle of the screen
  ...
  RCommand.setSegmentLength(9);                               // set segmentLength between points
  ...
  for (int i=0; i<numPointsText; i+=(int)(map(mouseY,0, height,1,10))) {
...
void draw() {
 ...
 fill(255,10);
  ...
  if (displayPoints) {
    ...
    interpolationBetweenPoints += 0.05;
