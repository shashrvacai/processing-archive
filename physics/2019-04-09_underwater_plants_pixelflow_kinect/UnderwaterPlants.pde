...
public void setup() {
  ...
  particles.param.col_A = new float[]{1f, 0.60f, 1f, 5};
...
void updateScene() {
  ...
  pg_obstacles.pushMatrix();
  {
    ...
    pg_obstacles.translate(mouseX, mouseY);        // obstacle position 
    pg_obstacles.rotate(frameCount/map(mouseX, 0, width,-60,60));   /// obstacle rotation
    pg_obstacles.fill(0, 255);  // the opacey is directly proportionate to he strenght of the obstacle    
    //pg_obstacles.rect(0, 0, 10, 2*dim);                          // ocstacle size 
  }
...
public void spawnParticles(float px_,float py_) {
