void draw_landscape(float aa_) {
  ...
  int xoff = (int)map(cc2,0,127,0,width);     // location of the center
  int yoff = (int)map(cc3,0,127,0,height);
  
  int xrad_step = int(map(cc4,0,127,4,60));      // spacing 
  int yrad_step = int(map(cc5,0,127,4,60));
  
  int xdeform = int(map(cc6,0,127,4,300));
  int ydeform = int(map(cc7,0,127,4,300));
  
  float xrmotion_size = 60;     // each particle rotation 
