...
int obFreq = 200 ;      /// dist between the Fans
float ampInten = 10;    // impact odf the sound DO NOT MAKE IT 0 ;
float cohMax = 10 ;     // play with this while sound check(not less than 1)
float colMax = 5 ;     // play with this while sound check(not less than 1)
...
public void draw() { 
  ...
  particles.param.mul_coh = map(aa, 0, 10, 0.01f, cohMax);
  particles.param.mul_col = map(aa, 0,10, 0.1f, 5.0f);
