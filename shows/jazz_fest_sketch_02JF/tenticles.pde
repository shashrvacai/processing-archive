class Tentacle
{
   int MAX_NEW_TENTACLES =1;//max nb of new tentacles at a time
   int NB_CIRCLES_MIN = 8;
   int NB_CIRCLES_MAX = 98;
   float DISTANCE_MIN = .25;//min percentage of the radius
  float DISTANCE_MAX = .4;//max percentage of the radius
  float DIST_VAR_MAX = .2;//max evolution of the distance
  float EVOL_RANDOM = .5;//percentage of big/small circles center's moves 
  int TENTACLE_SIZE_MIN = 18;//min number of circles per tentacle
  float DISPERTION_RATE = .065;//chance that a tentacle will split
