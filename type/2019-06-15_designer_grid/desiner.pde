...
void setup() {
  size(1920, 1080, P2D); // per vertex coloring requires an OpenGL renderer
  ...
  pg.text("designer", pg.width/2, pg.height/2); 
  pg.endDraw();
}
...
void draw() {
  ...
  float xx = (sin(a)*height/4);
  float yy = width/2;
  fill(0);
  ellipse(xx,yy,30,30);
  
  int gridVertical = (int) map(mouseY, 0, height, 10, 100);
  int gridHorizontal = (int) map(mouseY, 0, width, 10, 100);
